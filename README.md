...
Проект собирает публично доступные данные о вакансиях с hh.ru для аналитики рынка труда. Парсер охватывает широкую выборку через сегментацию по региону, опыту, формату работы и наличию зарплаты.
...
Парсер и дашборд разнесены по двум средам: парсинг запускается вручную с домашнего ноутбука (обычный IP, не под антибот-защитой), а Superset и DuckDB работают постоянно на отдельном удалённом сервере в Docker.
...
hh.ru
  └─► Selenium + BeautifulSoup   # парсинг страниц вакансий
        └─► pandas DataFrame      # обработка и дедупликация
              └─► CSV
                    └─► scp + SSH-триггер   # синхронизация со сервером

[Удалённый сервер, Docker]
              CSV ─► DuckDB       # импорт, дедуп по vacancy_id
                       └─► Apache Superset   # дашборд, доступ напрямую по IP:порту
```

Запуск парсера — по необходимости обновить данные, а не по расписанию: это осознанное решение, чтобы не создавать лишнюю нагрузку на hh.ru и не попадать под антибот-блокировки.

---

## Как устроен парсер

Парсер строит **декартово произведение** из 4 измерений — это обеспечивает полное покрытие рынка и репрезентативную выборку по всем сегментам аудитории:
...
| Обработка | pandas |
| Синхронизация | scp + SSH |
| Хранение | DuckDB |
| Визуализация | Apache Superset (Docker) |
| Доступ к дашборду | напрямую по IP:порту сервера |
